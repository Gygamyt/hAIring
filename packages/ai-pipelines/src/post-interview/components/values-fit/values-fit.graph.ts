...
import {
    createValuesFitGenerateNode,
    validateValuesFitNode,
    createValuesFitFixNode,
    handleFailureNode,
    routerForValuesFit,
} from './values-fit.nodes';
import {
    ValuesFitStateAnnotation,

} from './values-fit.state';

/**
 * Creates a robust LangGraph subgraph for analyzing values fit.
 *
 * This workflow implements a "Generate -> Validate -> Fix (with Retries)" loop.
 *
 * 1. `generateNode`: Attempts to parse the transcript against company values.
 ...
 */
export const createValuesFitSubgraph = (
    llm: ChatGoogleGenerativeAI,
) => {
    const generateNode = createValuesFitGenerateNode(llm);
    const fixNode = createValuesFitFixNode(llm);

    const workflow = new StateGraph(
        ValuesFitStateAnnotation,
    )
        .addNode('generateNode', generateNode)
        .addNode('validateNode', validateValuesFitNode)
    ...
    workflow
        ...
        .addConditionalEdges('validateNode', routerForValuesFit, {
            success: END,
